...
pub const Three = struct {
    ...
    pub fn init(arg: []const u8) !Three {
        if (arg.len != 7) return error.MissingPairArg;
    ...
    pub fn compute(self: Three, dice_numbers: [5]u8) !void {
        ...
        try std.io.getStdOut().writer().print("Chances to get a {d} three-of-a-kind: {d:.2}%\n", .{self.n, probability});
    }
};
